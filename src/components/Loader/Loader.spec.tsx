...
describe('Loader Component', () => {
  it('should render the loader with default props', () => {
    ...
    // then
    expect(screen.getByTestId('spinner')).toHaveClass('border-r-black');
    expect(screen.getByTestId('spinner')).toHaveClass('w-8');
  });

  it('should render different color if color prop is passed', () => {
    // given
    const color = 'violet';
    ...
    // then
    expect(screen.getByTestId('spinner')).toHaveClass(`border-r-${color}-500`);
  });

  it('should render color from colorHexCode prop if colorHexCode prop is passed', () => {
    // given
    const color = 'green';
    const colorHexCode = '#1abc9c';

    // when
    render(<Loader color={color} colorHexCode={colorHexCode} />);

    // then
    expect(screen.getByTestId('spinner')).not.toHaveClass(
      `border-r-${color}-500`
    );
    expect(screen.getByTestId('spinner')).toHaveStyle(
      `border-right-color: ${colorHexCode}}`
    );
  });
